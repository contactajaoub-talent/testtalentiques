...
export const Accompaniment = () => { const [open,setOpen]=useState(false); return (<>
    ...
    <div className="container mx-auto px-4 md:px-6 relative z-10">
      <div className="grid lg:grid-cols-[1.05fr_.95fr] gap-12 items-center max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          <button type="button" onClick={()=>setOpen(true)} className="ml-3 mt-3 inline-flex items-center gap-2 rounded-xl border border-white/30 px-6 py-3.5 font-bold text-white hover:bg-white/10">Demander un accompagnement <ArrowRight size={18}/></button>
        </motion.div>
...
<CRMModal config={open?{type:'accompagnement',title:'Demande d’accompagnement'}:null} onClose={()=>setOpen(false)}/>
</>); };
